...
Soustraction::Soustraction(Constante& pa, Constante& pb, int pModeConstante, int pModeComplexes) : OperationBinaire(pa, pb, pModeConstante, pModeComplexes)
{
}

Constante* Soustraction::getValue() const {
    ...
    if (expa != NULL){
        ...
        if (expb != NULL){
            ...
            return soustraction(*expa, *expb);
        }
        else {
            Complexe* complexeb = dynamic_cast<Complexe*>(b);
            if (complexeb != NULL) {
                // b est un complexe
                return soustraction(*expa, *complexeb);
            }
            else {
                // b est un rationnel
                return soustraction(*expa, *((Rationnel*)b));
            }
        }
    }
    else {
        ...
        if (complexea != NULL){
            ...
            if (expb != NULL) {
                // b est une expression
                return soustraction(*complexea, *expb);
            }
            else {
                Complexe* complexeb = dynamic_cast<Complexe*>(b);
                if (complexeb != NULL){
                    //b est un complexe
                    return soustraction(*complexea, *complexeb);
                }
                else{
                    return soustraction(*complexea, *((Rationnel*)b));
                }
           }
        }
        else{
            ...
            if (expb != NULL) {
                // b est une expression
                return soustraction(*((Rationnel*)a), *expb);
            }
            else {
                Complexe* complexeb = dynamic_cast<Complexe*>(b);
                if (complexeb != NULL){
                    //b est un complexe
                    return soustraction(*((Rationnel*)a), *complexeb);
                }
                else{
                    return soustraction(*((Rationnel*)a), *((Rationnel*)b));
                }
           }
        }
    }
...
Constante* Soustraction::soustraction(const Rationnel& a, const Rationnel& b) const {
    Constante* retour;
...
Constante* Soustraction::soustraction(const Complexe& c1, const Complexe& c2) const {
    Constante* retour;
    switch(mModeConstante) {
        case CalculatricePolonaise::MODE_ENTIER:
            if (mModeComplexes == CalculatricePolonaise::MODE_SANS_COMPLEXES) {
                ...
                if (c1.isFloat()) {
                    ...
                    if (c2.isFloat()) {
                        retour = new Complexe(Rationnel((float)c1.getReRationnel().getNum()/(float)c1.getReRationnel().getDen() - c2.getRe()),
                                              Rationnel((float)c1.getImRationnel().getNum()/(float)c1.getImRationnel().getDen() - c2.getIm()));
                    }
                    else {
            ...
            if (mModeComplexes == CalculatricePolonaise::MODE_SANS_COMPLEXES) {
                ...
                if (c1.isFloat()) {
                    ...
                    if (c2.isFloat()) {
                        ...
                        retour = new Complexe(Rationnel(rRe.getDen()*c1.getReRationnel().getNum() - rRe.getNum()*c1.getReRationnel().getDen(), rRe.getDen()*c1.getReRationnel().getDen()),
                                                        Rationnel(rIm.getDen()*c1.getImRationnel().getNum() - rIm.getNum()*c1.getImRationnel().getDen(), rIm.getDen()*c1.getImRationnel().getDen()));
                    }
                    else {
        ...
        case CalculatricePolonaise::MODE_REEL:
            if (mModeComplexes == CalculatricePolonaise::MODE_SANS_COMPLEXES) {
                ...
                if (c1.isFloat()) {
                    ...
                    if (c2.isFloat()) {
                        retour = new Complexe((float)c1.getReRationnel().getNum()/(float)c1.getReRationnel().getDen() - c2.getRe(), (float)c1.getImRationnel().getNum()/(float)c1.getImRationnel().getDen() - c2.getIm());
                    }
                    else {
...
Constante* Soustraction::soustraction(const Complexe& a, const Rationnel& b) const {
    const Complexe& r(b);
    return soustraction(a,r);
}

Constante* Soustraction::soustraction(const Rationnel& ra, const Complexe& cb) const {

    return soustraction(cb, ra);
}

Constante* Soustraction::soustraction(const Expression& e1, const Expression& e2) const {
    return new Expression(e1.getExp() + " " + e2.getExp() + " -");
}

Constante* Soustraction::soustraction(const Expression& e1, const Rationnel& r) const {
    return new Expression(e1.getExp() + " " + r.toString() + " -");
}

Constante* Soustraction::soustraction(const Expression& e1, const Complexe& c) const {
    return new Expression(e1.getExp() + " " + c.toString() + " -");
}

Constante* Soustraction::soustraction(const Rationnel& r, const Expression& e1) const {
    return new Expression(r.toString() + " " + e1.getExp() + " -");
}

Constante* Soustraction::soustraction(const Complexe& c, const Expression& e1) const {
